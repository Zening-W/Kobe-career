import {
  ...
  selector_default,
  styleValue
} from "./chunk-HRFNCIBP.js";
import {
  path_default
} from "./chunk-CJRJVPPZ.js";
import {
  axisBottom,
  axisLeft,
  axisRight,
  axisTop
} from "./chunk-P4ZYMOQK.js";
import {
  autoType,
  blob_default,
  buffer_default,
...
function blur(values, r) {
  ...
  blur4(values, temp, 0, length3, 1);
  blur4(temp, values, 0, length3, 1);
  blur4(values, temp, 0, length3, 1);
  return values;
}
...
function reducer(reduce3) {
  return (values) => reduce3(...values);
}
function cross(...values) {
  const reduce3 = typeof values[values.length - 1] === "function" && reducer(values.pop());
...
function cumsum(values, valueof) {
  var sum5 = 0, index4 = 0;
  return Float64Array.from(values, valueof === void 0 ? (v2) => sum5 += +v2 || 0 : (v2) => sum5 += +valueof(v2, index4++, values) || 0);
}
...
function flatRollup(values, reduce3, ...keys) {
  return flatten(rollups(values, reduce3, ...keys), keys);
}
function rollup(values, reduce3, ...keys) {
  return nest(values, identity2, reduce3, keys);
}
function rollups(values, reduce3, ...keys) {
  return nest(values, Array.from, reduce3, keys);
}
...
function nest(values, map6, reduce3, keys) {
  return function regroup(values2, i) {
    ...
    for (const value of values2) {
      const key = keyof(value, ++index4, values2);
      const group3 = groups3.get(key);
      if (group3)
        group3.push(value);
      else
...
function groupSort(values, reduce3, key) {
  return (reduce3.length !== 2 ? sort(rollup(values, reduce3, key), ([ak, av], [bk, bv]) => ascending(av, bv) || ascending(ak, bk)) : sort(group(values, key), ([ak, av], [bk, bv]) => reduce3(av, bv) || ascending(ak, bk))).map(([key2]) => key2);
}
...
function min(values, valueof) {
  ...
  if (valueof === void 0) {
    ...
    let index4 = -1;
    for (let value of values) {
      if ((value = valueof(value, ++index4, values)) != null && (min5 > value || min5 === void 0 && value >= value)) {
        min5 = value;
      }
    }
  }
...
function quickselect(array7, k2, left = 0, right = Infinity, compare) {
  k2 = Math.floor(k2);
  left = Math.floor(Math.max(0, left));
  right = Math.floor(Math.min(array7.length - 1, right));
  if (!(left <= k2 && k2 <= right))
    return array7;
  ...
  while (right > left) {
    if (right - left > 600) {
      ...
      quickselect(array7, k2, newLeft, newRight, compare);
    }
    const t = array7[k2];
    let i = left;
    let j = right;
    swap(array7, left, k2);
    if (compare(array7[right], t) > 0)
      swap(array7, left, right);
    while (i < j) {
      swap(array7, i, j), ++i, --j;
    ...
    if (compare(array7[left], t) === 0)
      swap(array7, left, j);
    else
      ++j, swap(array7, j, right);
    if (j <= k2)
      left = j + 1;
    if (k2 <= j)
      right = j - 1;
...
function quantileIndex(values, p, valueof = number) {
  if (isNaN(p = +p))
    return;
  numbers3 = Float64Array.from(values, (_, i2) => number(valueof(values[i2], i2, values)));
  if (p <= 0)
    return minIndex(numbers3);
  if (p >= 1)
    return maxIndex(numbers3);
  var numbers3, index4 = Uint32Array.from(values, (_, i2) => i2), j = numbers3.length - 1, i = Math.floor(j * p);
  quickselect(index4, i, 0, j, (i2, j2) => ascendingDefined(numbers3[i2], numbers3[j2]));
  i = greatest(index4.subarray(0, i + 1), (i2) => numbers3[i2]);
  return i >= 0 ? i : -1;
}
...
function thresholdFreedmanDiaconis(values, min5, max6) {
  const c6 = count(values), d = quantile2(values, 0.75) - quantile2(values, 0.25);
  return c6 && d ? Math.ceil((max6 - min5) / (2 * d * Math.pow(c6, -1 / 3))) : 1;
}
...
function thresholdScott(values, min5, max6) {
  ...
  return c6 && d ? Math.ceil((max6 - min5) * Math.cbrt(c6) / (3.49 * d)) : 1;
}
...
function scan(values, compare) {
  const index4 = leastIndex(values, compare);
...
function shuffler(random) {
  return function shuffle2(array7, i0 = 0, i1 = array7.length) {
    let m3 = i1 - (i0 = +i0);
    while (m3) {
      const i = random() * m3-- | 0, t = array7[m3 + i0];
      array7[m3 + i0] = array7[i + i0];
      array7[i + i0] = t;
    }
    return array7;
  };
...
function transpose(matrix) {
  if (!(n = matrix.length))
    return [];
  for (var i = -1, m3 = min(matrix, length2), transpose3 = new Array(m3); ++i < m3; ) {
...
function merge_default(context) {
  ...
  for (var groups0 = this._groups, groups1 = selection4._groups, m0 = groups0.length, m1 = groups1.length, m3 = Math.min(m0, m1), merges = new Array(m0), j = 0; j < m3; ++j) {
...
function style_default(name, value, priority) {
  return arguments.length > 1 ? this.each((value == null ? styleRemove : typeof value === "function" ? styleFunction : styleConstant)(name, value, priority == null ? "" : priority)) : styleValue2(this.node(), name);
}
...
function insert_default(name, before) {
  var create2 = typeof name === "function" ? name : creator_default(name), select = before == null ? constantNull : typeof before === "function" ? before : selector_default2(before);
  return this.select(function() {
...
function select_default3(selector) {
  return typeof selector === "string" ? new Selection([[document.querySelector(selector)]], [document.documentElement]) : new Selection([[selector]], root);
}

// node_modules/d3-drag/node_modules/d3-selection/src/local.js
var nextId = 0;
function local() {
  return new Local();
}
...
// node_modules/d3-drag/node_modules/d3-selection/src/sourceEvent.js
...
// node_modules/d3-drag/node_modules/d3-selection/src/pointer.js
...
function nodrag_default(view) {
  var root5 = view.document.documentElement, selection4 = select_default3(view).on("dragstart.drag", noevent_default, nonpassivecapture);
  if ("onselectstart" in root5) {
    selection4.on("selectstart.drag", noevent_default, nonpassivecapture);
  } else {
...
function yesdrag(view, noclick) {
  var root5 = view.document.documentElement, selection4 = select_default3(view).on("dragstart.drag", null);
  if (noclick) {
    selection4.on("click.drag", noevent_default, nonpassivecapture);
    setTimeout(function() {
...
function drag_default() {
  ...
  function drag(selection4) {
    selection4.on("mousedown.drag", mousedowned).filter(touchable).on("touchstart.drag", touchstarted).on("touchmove.drag", touchmoved, nonpassive).on("touchend.drag touchcancel.drag", touchended).style("touch-action", "none").style("-webkit-tap-highlight-color", "rgba(0,0,0,0)");
  }
  function mousedowned(event, d) {
    if (touchending || !filter6.call(this, event, d))
      return;
    var gesture = beforestart(this, container.call(this, event, d), event, d, "mouse");
    if (!gesture)
      return;
    select_default3(event.view).on("mousemove.drag", mousemoved, nonpassivecapture).on("mouseup.drag", mouseupped, nonpassivecapture);
  ...
  function mouseupped(event) {
    select_default3(event.view).on("mousemove.drag mouseup.drag", null);
    yesdrag(event.view, mousemoving);
    noevent_default(event);
    gestures.mouse("end", event);
  }
  function touchstarted(event, d) {
    if (!filter6.call(this, event, d))
      return;
  ...
  function beforestart(that, container2, event, d, identifier, touch) {
    var dispatch2 = listeners.copy(), p = pointer_default2(touch || event, container2), dx, dy, s2;
  ...
  drag.filter = function(_) {
    return arguments.length ? (filter6 = typeof _ === "function" ? _ : constant_default2(!!_), drag) : filter6;
  };
...
function bindIndex2(parent, group3, enter, update, exit, data) {
  var i = 0, node, groupLength = group3.length, dataLength = data.length;
  for (; i < dataLength; ++i) {
    if (node = group3[i]) {
      node.__data__ = data[i];
...
function bindKey2(parent, group3, enter, update, exit, data, key) {
  var i, node, nodeByKeyValue = /* @__PURE__ */ new Map(), groupLength = group3.length, dataLength = data.length, keyValues = new Array(groupLength), keyValue;
  for (i = 0; i < groupLength; ++i) {
    if (node = group3[i]) {
      keyValues[i] = keyValue = key.call(node, node.__data__, i, group3) + "";
...
function data_default2(value, key) {
  ...
  for (var m3 = groups3.length, update = new Array(m3), enter = new Array(m3), exit = new Array(m3), j = 0; j < m3; ++j) {
    var parent = parents[j], group3 = groups3[j], groupLength = group3.length, data = arraylike2(value.call(parent, parent && parent.__data__, j, parents)), dataLength = data.length, enterGroup = enter[j] = new Array(dataLength), updateGroup = update[j] = new Array(dataLength), exitGroup = exit[j] = new Array(groupLength);
    bind(parent, group3, enterGroup, updateGroup, exitGroup, data, key);
    for (var i0 = 0, i1 = 0, previous, next; i0 < dataLength; ++i0) {
      if (previous = enterGroup[i0]) {
        if (i0 >= i1)
...
function style_default2(name, value, priority) {
  return arguments.length > 1 ? this.each((value == null ? styleRemove2 : typeof value === "function" ? styleFunction2 : styleConstant2)(name, value, priority == null ? "" : priority)) : styleValue3(this.node(), name);
}
...
function insert_default2(name, before) {
  var create2 = typeof name === "function" ? name : creator_default2(name), select = before == null ? constantNull2 : typeof before === "function" ? before : selector_default3(before);
  return this.select(function() {
    return this.insertBefore(create2.apply(this, arguments), select.apply(this, arguments) || null);
  });
}
...
var root2 = [null];
function Selection2(groups3, parents) {
  this._groups = groups3;
  this._parents = parents;
}
function selection2() {
...
function select_default5(selector) {
  return typeof selector === "string" ? new Selection2([[document.querySelector(selector)]], [document.documentElement]) : new Selection2([[selector]], root2);
}
...
function pointer_default3(event, node) {
...
function interrupt_default(node, name) {
  var schedules = node.__transition, schedule, active, empty6 = true, i;
  if (!schedules)
    return;
...
function selectAll_default5(select) {
  ...
  for (var groups3 = this._groups, m3 = groups3.length, subgroups = [], parents = [], j = 0; j < m3; ++j) {
    for (var group3 = groups3[j], n = group3.length, node, i = 0; i < n; ++i) {
      if (node = group3[i]) {
        for (var children4 = select.call(node, node.__data__, i, group3), child, inherit2 = get2(node, id2), k2 = 0, l = children4.length; k2 < l; ++k2) {
          if (child = children4[k2]) {
            schedule_default(child, name, id2, k2, children4, inherit2);
          }
        }
...
function styleMaybeRemove(id2, name) {
  ...
  return function() {
    var schedule = set3(this, id2), on = schedule.on, listener = schedule.value[key] == null ? remove4 || (remove4 = styleRemove3(name)) : void 0;
  ...
  if (schedules) {
    ...
    for (i in schedules) {
      if ((schedule = schedules[i]).state > SCHEDULED && schedule.name === name) {
        return new Transition([[node]], root3, name, +i);
      }
    }
  }
  return null;
...
function BrushEvent(type2, {
  ...
  Object.defineProperties(this, {
    ...
    sourceEvent: { value: sourceEvent, enumerable: true, configurable: true },
    target: { value: target, enumerable: true, configurable: true },
    selection: { value: selection4, enumerable: true, configurable: true },
...
function local3(node) {
  while (!node.__brush)
    if (!(node = node.parentNode))
      return;
  return node.__brush;
}
function empty4(extent3) {
...
function brush(dim) {
  var extent3 = defaultExtent, filter6 = defaultFilter2, touchable = defaultTouchable2, keys = true, listeners = dispatch_default("start", "brush", "end"), handleSize = 6, touchending;
  function brush2(group3) {
    ...
    overlay.enter().append("rect").attr("class", "overlay").attr("pointer-events", "all").attr("cursor", cursors.overlay).merge(overlay).each(function() {
      var extent4 = local3(this).extent;
      select_default5(this).attr("x", extent4[0][0]).attr("y", extent4[0][1]).attr("width", extent4[1][0] - extent4[0][0]).attr("height", extent4[1][1] - extent4[0][1]);
    });
  ...
  brush2.move = function(group3, selection4, event) {
    if (group3.tween) {
  ...
  function redraw() {
    var group3 = select_default5(this), selection4 = local3(this).selection;
    if (selection4) {
      group3.selectAll(".selection").style("display", null).attr("x", selection4[0][0]).attr("y", selection4[0][1]).attr("width", selection4[1][0] - selection4[0][0]).attr("height", selection4[1][1] - selection4[0][1]);
  ...
  function started(event) {
    if (touchending && !event.touches)
      return;
    if (!filter6.apply(this, arguments))
      return;
    var that = this, type2 = event.target.__data__.type, mode3 = (keys && event.metaKey ? type2 = "overlay" : type2) === "selection" ? MODE_DRAG : keys && event.altKey ? MODE_CENTER : MODE_HANDLE, signX = dim === Y ? null : signsX[type2], signY = dim === X ? null : signsY[type2], state = local3(that), extent4 = state.extent, selection4 = state.selection, W = extent4[0][0], w0, w1, N = extent4[0][1], n0, n1, E = extent4[1][0], e0, e1, S = extent4[1][1], s0, s1, dx = 0, dy = 0, moving, shifting = signX && signY && keys && event.shiftKey, lockX, lockY, points = Array.from(event.touches || [event], (t) => {
      const i = t.identifier;
      t = pointer_default3(t, that);
      t.point0 = t.slice();
      t.identifier = i;
      return t;
    });
    ...
    e1 = e0;
    s1 = s0;
    var group3 = select_default5(that).attr("pointer-events", "none");
    ...
    function move(event2) {
      ...
      if (state.selection)
        selection4 = state.selection;
      if (lockX)
        w1 = selection4[0][0], e1 = selection4[1][0];
      if (lockY)
        n1 = selection4[0][1], s1 = selection4[1][1];
  ...
  function initialize() {
    ...
    state.extent = number2(extent3.apply(this, arguments));
    state.dim = dim;
    return state;
  }
  brush2.extent = function(_) {
    return arguments.length ? (extent3 = typeof _ === "function" ? _ : constant_default4(number2(_)), brush2) : extent3;
  };
  brush2.filter = function(_) {
    return arguments.length ? (filter6 = typeof _ === "function" ? _ : constant_default4(!!_), brush2) : filter6;
  };
...
function bisector2(f) {
  ...
  if (f.length !== 2) {
    compare1 = ascending4;
    compare2 = (d, x4) => ascending4(f(d), x4);
    delta = (d, x4) => f(d) - x4;
  } else {
    compare1 = f === ascending4 || f === descending2 ? f : zero2;
    compare2 = f;
    delta = f;
  }
  function left(a4, x4, lo = 0, hi = a4.length) {
...
var e22 = Math.sqrt(2);
function tickSpec2(start2, stop, count4) {
  const step = (stop - start2) / Math.max(0, count4), power = Math.floor(Math.log10(step)), error = step / Math.pow(10, power), factor = error >= e102 ? 10 : error >= e52 ? 5 : error >= e22 ? 2 : 1;
  let i1, i2, inc2;
  if (power < 0) {
    inc2 = Math.pow(10, -power) / factor;
    i1 = Math.round(start2 * inc2);
  ...
  if (i2 < i1 && 0.5 <= count4 && count4 < 2)
    return tickSpec2(start2, stop, count4 * 2);
  return [i1, i2, inc2];
}
function ticks2(start2, stop, count4) {
  ...
  if (start2 === stop)
    return [start2];
  const reverse3 = stop < start2, [i1, i2, inc2] = reverse3 ? tickSpec2(stop, start2, count4) : tickSpec2(start2, stop, count4);
  if (!(i2 >= i1))
    return [];
  const n = i2 - i1 + 1, ticks3 = new Array(n);
  if (reverse3) {
...
function nice2(start2, stop, count4) {
  let prestep;
  while (true) {
    const step = tickIncrement2(start2, stop, count4);
    if (step === prestep || step === 0 || !isFinite(step)) {
      return [start2, stop];
    } else if (step > 0) {
      start2 = Math.floor(start2 / step) * step;
...
function thresholdSturges2(values) {
  return Math.max(1, Math.ceil(Math.log(count2(values)) / Math.LN2) + 1);
}
...
function contours_default() {
  var dx = 1, dy = 1, threshold2 = thresholdSturges2, smooth = smoothLinear;
  function contours(values) {
    var tz = threshold2(values);
    if (!Array.isArray(tz)) {
      const e = extent2(values, finite);
      tz = ticks2(...nice2(e[0], e[1], tz), tz);
  ...
  function isorings(values, value, callback) {
    ...
    function stitch(line) {
      var start2 = [line[0][0] + x4, line[0][1] + y4], end = [line[1][0] + x4, line[1][1] + y4], startIndex = index4(start2), endIndex = index4(end), f, g;
...
function density_default() {
  ...
  function grid(data) {
    ...
    blur22({ data: values, width: n, height: m3 }, r * pow2k);
    return values;
  }
  function density(data) {
    var values = grid(data), tz = threshold2(values), pow4k = Math.pow(2, 2 * k2);
    if (!Array.isArray(tz)) {
      tz = ticks2(Number.MIN_VALUE, max4(values) / pow4k, tz);
    }
  ...
  density.contours = function(data) {
    ...
    Object.defineProperty(contour, "max", { get: () => max4(values) / pow4k });
    return contour;
  };
...
function link_default(links) {
  var id2 = index3, strength = defaultStrength, strengths, distance = constant_default7(30), distances, nodes, count4, bias, random, iterations2 = 1;
  if (links == null)
    links = [];
...
function locale_default(locale3) {
  var group3 = locale3.grouping === void 0 || locale3.thousands === void 0 ? identity_default : formatGroup_default(map5.call(locale3.grouping, Number), locale3.thousands + ""), currencyPrefix = locale3.currency === void 0 ? "" : locale3.currency[0] + "", currencySuffix = locale3.currency === void 0 ? "" : locale3.currency[1] + "", decimal = locale3.decimal === void 0 ? "." : locale3.decimal + "", numerals = locale3.numerals === void 0 ? identity_default : formatNumerals_default(map5.call(locale3.numerals, String)), percent = locale3.percent === void 0 ? "%" : locale3.percent + "", minus = locale3.minus === void 0 ? "−" : locale3.minus + "", nan = locale3.nan === void 0 ? "NaN" : locale3.nan + "";
...
function circle_default() {
  var center = constant_default8([0, 0]), radius = constant_default8(90), precision = constant_default8(2), ring, rotate, stream = { point: point6 };
  ...
  circle.center = function(_) {
    return arguments.length ? (center = typeof _ === "function" ? _ : constant_default8([+_[0], +_[1]]), circle) : center;
  };
...
function pointEqual_default(a4, b) {
  return abs3(a4[0] - b[0]) < epsilon4 && abs3(a4[1] - b[1]) < epsilon4;
}
...
function polygonContains_default(polygon, point6) {
  ...
  return (angle2 < -epsilon4 || angle2 < epsilon4 && sum5 < -epsilon22) ^ winding & 1;
}
...
function compareIntersection(a4, b) {
  return ((a4 = a4.x)[0] < 0 ? a4[1] - halfPi3 - epsilon4 : halfPi3 - a4[1]) - ((b = b.x)[0] < 0 ? b[1] - halfPi3 - epsilon4 : halfPi3 - b[1]);
}
...
function graticuleX(y06, y12, dy) {
  var y4 = range_default(y06, y12 - epsilon4, dy).concat(y12);
  return function(x4) {
...
function graticuleY(x06, x12, dx) {
  var x4 = range_default(x06, x12 - epsilon4, dx).concat(x12);
  return function(y4) {
...
function resample(project, delta2) {
  function resampleLineTo(x06, y06, lambda04, a0, b02, c0, x12, y12, lambda12, a1, b12, c1, depth, stream) {
    ...
    if (d2 > 4 * delta2 && depth--) {
      var a4 = a0 + a1, b = b02 + b12, c6 = c0 + c1, m3 = sqrt2(a4 * a4 + b * b + c6 * c6), phi2 = asin(c6 /= m3), lambda22 = abs3(abs3(c6) - 1) < epsilon4 || abs3(lambda04 - lambda12) < epsilon4 ? (lambda04 + lambda12) / 2 : atan2(b, a4), p = project(lambda22, phi2), x22 = p[0], y22 = p[1], dx2 = x22 - x06, dy2 = y22 - y06, dz = dy * dx2 - dx * dy2;
...
function albersUsa_default() {
  ...
  albersUsa.translate = function(_) {
    ...
    alaskaPoint = alaska.translate([x4 - 0.307 * k2, y4 + 0.201 * k2]).clipExtent([[x4 - 0.425 * k2 + epsilon4, y4 + 0.12 * k2 + epsilon4], [x4 - 0.214 * k2 - epsilon4, y4 + 0.234 * k2 - epsilon4]]).stream(pointStream);
    hawaiiPoint = hawaii.translate([x4 - 0.205 * k2, y4 + 0.212 * k2]).clipExtent([[x4 - 0.214 * k2 + epsilon4, y4 + 0.166 * k2 + epsilon4], [x4 - 0.115 * k2 - epsilon4, y4 + 0.234 * k2 - epsilon4]]).stream(pointStream);
    return reset();
  };
...
function cluster_default() {
  ...
  function cluster(root5) {
    ...
    var left = leafLeft(root5), right = leafRight(root5), x06 = left.x - separation(left, right) / 2, x12 = right.x + separation(right, left) / 2;
...
function count3(node) {
  var sum5 = 0, children4 = node.children, i = children4 && children4.length;
  if (!i)
    sum5 = 1;
  else
    while (--i >= 0)
      sum5 += children4[i].value;
  node.value = sum5;
}
...
function sum_default(value) {
  return this.eachAfter(function(node) {
    var sum5 = +value(node.data) || 0, children4 = node.children, i = children4 && children4.length;
    while (--i >= 0)
      sum5 += children4[i].value;
    node.value = sum5;
  });
...
function pack_default() {
  ...
  function pack(root5) {
    ...
    if (radius) {
      root5.eachBefore(radiusLeaf(radius)).eachAfter(packChildrenRandom(padding, 0.5, random)).eachBefore(translateChild(1));
    } else {
...
function packChildrenRandom(padding, k2, random) {
  return function(node) {
    if (children4 = node.children) {
      var children4, i, n = children4.length, r = padding(node) * k2 || 0, e;
      if (r)
        for (i = 0; i < n; ++i)
          children4[i].r += r;
      e = packSiblingsRandom(children4, random);
      if (r)
        for (i = 0; i < n; ++i)
          children4[i].r -= r;
      node.r = e + r;
...
function stratify_default() {
  ...
  function stratify(data) {
    var nodes = Array.from(data), currentId = id2, currentParentId = parentId, n, d, i, root5, parent, node, nodeId, nodeKey, nodeByKey = /* @__PURE__ */ new Map();
    if (path2 != null) {
...
function executeShifts(v2) {
  var shift = 0, change = 0, children4 = v2.children, i = children4.length, w;
  while (--i >= 0) {
    w = children4[i];
    w.z += shift;
    w.m += shift;
...
function treeRoot(root5) {
  var tree = new TreeNode(root5, 0), node, nodes = [tree], child, children4, i, n;
  while (node = nodes.pop()) {
...
function tree_default() {
  ...
  function tree(root5) {
    ...
    if (nodeSize)
      ...
      var left = root5, right = root5, bottom = root5;
  ...
  function firstWalk(v2) {
    var children4 = v2.children, siblings = v2.parent.children, w = v2.i ? siblings[v2.i - 1] : null;
    if (children4) {
      executeShifts(v2);
...
var Path2 = class {
  ...
  arc(x4, y4, r, a0, a1, ccw) {
    ...
    if (da2 > tauEpsilon) {
      ...
      this._append`A${r},${r},0,${+(da2 >= pi4)},${cw},${this._x1 = x4 + r * Math.cos(a1)},${this._y1 = y4 + r * Math.sin(a1)}`;
    }
  }
  rect(x4, y4, w, h) {
...
function namespace_default4(name) {
  var prefix = name += "", i = prefix.indexOf(":");
  if (i >= 0 && (prefix = name.slice(0, i)) !== "xmlns")
    name = name.slice(i + 1);
...
function creator_default3(name) {
  var fullname = namespace_default4(name);
...
function select_default7(select) {
  ...
  return new Selection4(subgroups, this._parents);
}
...
function arrayAll3(select) {
  return function() {
    return array6(select.apply(this, arguments));
  };
}
function selectAll_default6(select) {
  ...
  return new Selection4(subgroups, parents);
}
...
function enter_default3() {
  return new Selection4(this._enter || this._groups.map(sparse_default3), this._parents);
}
...
function bindIndex3(parent, group3, enter, update, exit, data) {
  var i = 0, node, groupLength = group3.length, dataLength = data.length;
  for (; i < dataLength; ++i) {
    if (node = group3[i]) {
      node.__data__ = data[i];
...
function bindKey3(parent, group3, enter, update, exit, data, key) {
  var i, node, nodeByKeyValue = /* @__PURE__ */ new Map(), groupLength = group3.length, dataLength = data.length, keyValues = new Array(groupLength), keyValue;
  for (i = 0; i < groupLength; ++i) {
    if (node = group3[i]) {
      keyValues[i] = keyValue = key.call(node, node.__data__, i, group3) + "";
...
function data_default4(value, key) {
  ...
  var bind = key ? bindKey3 : bindIndex3, parents = this._parents, groups3 = this._groups;
  if (typeof value !== "function")
    value = constant_default10(value);
  for (var m3 = groups3.length, update = new Array(m3), enter = new Array(m3), exit = new Array(m3), j = 0; j < m3; ++j) {
    var parent = parents[j], group3 = groups3[j], groupLength = group3.length, data = arraylike3(value.call(parent, parent && parent.__data__, j, parents)), dataLength = data.length, enterGroup = enter[j] = new Array(dataLength), updateGroup = update[j] = new Array(dataLength), exitGroup = exit[j] = new Array(groupLength);
    bind(parent, group3, enterGroup, updateGroup, exitGroup, data, key);
    for (var i0 = 0, i1 = 0, previous, next; i0 < dataLength; ++i0) {
      if (previous = enterGroup[i0]) {
        if (i0 >= i1)
  ...
  update = new Selection4(update, parents);
  update._enter = enter;
  update._exit = exit;
  return update;
}
...
function exit_default3() {
  return new Selection4(this._exit || this._groups.map(sparse_default3), this._parents);
}
...
function merge_default4(context) {
  ...
  return new Selection4(merges, this._parents);
}
...
function sort_default4(compare) {
  ...
  return new Selection4(sortgroups, this._parents).order();
}
function ascending5(a4, b) {
  return a4 < b ? -1 : a4 > b ? 1 : a4 >= b ? 0 : NaN;
}
...
function attrConstant4(name, value) {
  return function() {
    this.setAttribute(name, value);
  };
}
function attrConstantNS4(fullname, value) {
  return function() {
    this.setAttributeNS(fullname.space, fullname.local, value);
  };
}
function attrFunction4(name, value) {
  return function() {
    var v2 = value.apply(this, arguments);
    if (v2 == null)
      this.removeAttribute(name);
...
function attrFunctionNS4(fullname, value) {
  return function() {
    var v2 = value.apply(this, arguments);
    if (v2 == null)
      this.removeAttributeNS(fullname.space, fullname.local);
...
function attr_default4(name, value) {
  ...
  return this.each((value == null ? fullname.local ? attrRemoveNS4 : attrRemove4 : typeof value === "function" ? fullname.local ? attrFunctionNS4 : attrFunction4 : fullname.local ? attrConstantNS4 : attrConstant4)(fullname, value));
}
...
function styleConstant4(name, value, priority) {
  return function() {
    this.style.setProperty(name, value, priority);
  };
}
function styleFunction4(name, value, priority) {
  return function() {
    var v2 = value.apply(this, arguments);
    if (v2 == null)
      this.style.removeProperty(name);
...
function style_default4(name, value, priority) {
  return arguments.length > 1 ? this.each((value == null ? styleRemove4 : typeof value === "function" ? styleFunction4 : styleConstant4)(name, value, priority == null ? "" : priority)) : styleValue4(this.node(), name);
}
function styleValue4(node, name) {
...
function text_default5(value) {
  return arguments.length ? this.each(value == null ? textRemove3 : (typeof value === "function" ? textFunction4 : textConstant4)(value)) : this.node().textContent;
}
...
function append_default3(name) {
  var create2 = typeof name === "function" ? name : creator_default3(name);
  return this.select(function() {
    return this.appendChild(create2.apply(this, arguments));
  });
}
...
function insert_default3(name, before) {
  var create2 = typeof name === "function" ? name : creator_default3(name), select = before == null ? constantNull3 : typeof before === "function" ? before : selector_default4(before);
  return this.select(function() {
    return this.insertBefore(create2.apply(this, arguments), select.apply(this, arguments) || null);
  });
}
...
function on_default4(typename, value, options) {
...
var root4 = [null];
function Selection4(groups3, parents) {
  this._groups = groups3;
  this._parents = parents;
}
function selection3() {
  return new Selection4([[document.documentElement]], root4);
}
function selection_selection3() {
  return this;
}
Selection4.prototype = selection3.prototype = {
  constructor: Selection4,
...
function select_default8(selector) {
  return typeof selector === "string" ? new Selection4([[document.querySelector(selector)]], [document.documentElement]) : new Selection4([[selector]], root4);
}

// node_modules/d3/node_modules/d3-selection/src/create.js
function create_default3(name) {
  return select_default8(creator_default3(name).call(document.documentElement));
}
...
function sourceEvent_default3(event) {
  let sourceEvent;
  while (sourceEvent = event.sourceEvent)
    event = sourceEvent;
  return event;
...
function pointer_default4(event, node) {
  event = sourceEvent_default3(event);
  if (node === void 0)
    node = event.currentTarget;
  if (node) {
    var svg2 = node.ownerSVGElement || node;
...
function pointers_default3(events, node) {
  if (events.target) {
    events = sourceEvent_default3(events);
    if (node === void 0)
      node = events.currentTarget;
    events = events.touches || [events];
  }
  return Array.from(events, (event) => pointer_default4(event, node));
}

// node_modules/d3/node_modules/d3-selection/src/selectAll.js
function selectAll_default7(selector) {
  return typeof selector === "string" ? new Selection4([document.querySelectorAll(selector)], [document.documentElement]) : new Selection4([array6(selector)], root4);
}
...
function cornerTangents(x06, y06, x12, y12, r1, rc, cw) {
  var x01 = x06 - x12, y01 = y06 - y12, lo = (cw ? rc : -rc) / sqrt3(x01 * x01 + y01 * y01), ox = lo * y01, oy = -lo * x01, x11 = x06 + ox, y11 = y06 + oy, x10 = x12 + ox, y10 = y12 + oy, x004 = (x11 + x10) / 2, y004 = (y11 + y10) / 2, dx = x10 - x11, dy = y10 - y11, d2 = dx * dx + dy * dy, r = r1 - rc, D2 = x11 * y10 - x10 * y11, d = (dy < 0 ? -1 : 1) * sqrt3(max5(0, r * r * d2 - D2 * D2)), cx0 = (D2 * dy - dx * d) / d2, cy0 = (-D2 * dx - dy * d) / d2, cx1 = (D2 * dy + dx * d) / d2, cy1 = (-D2 * dx + dy * d) / d2, dx0 = cx0 - x004, dy0 = cy0 - y004, dx1 = cx1 - x004, dy1 = cy1 - y004;
...
function arc_default() {
  var innerRadius = arcInnerRadius, outerRadius = arcOuterRadius, cornerRadius = constant_default11(0), padRadius = null, startAngle = arcStartAngle, endAngle = arcEndAngle, padAngle = arcPadAngle, context = null, path2 = withPath(arc);
  function arc() {
    ...
    if (!(r1 > epsilon6))
      ...
      var a01 = a0, a11 = a1, a00 = a0, a10 = a1, da0 = da2, da1 = da2, ap = padAngle.apply(this, arguments) / 2, rp = ap > epsilon6 && (padRadius ? +padRadius.apply(this, arguments) : sqrt3(r0 * r0 + r1 * r1)), rc = min4(abs4(r1 - r0) / 2, +cornerRadius.apply(this, arguments)), rc0 = rc, rc1 = rc, t02, t12;
      if (rp > epsilon6) {
...
function line_default2(x4, y4) {
  var defined = constant_default11(true), context = null, curve = linear_default, output = null, path2 = withPath(line);
...
function area_default5(x06, y06, y12) {
  var x12 = null, defined = constant_default11(true), context = null, curve = linear_default, output = null, path2 = withPath(area);
...
function pie_default() {
  var value = identity_default4, sortValues = descending_default, sort3 = null, startAngle = constant_default11(0), endAngle = constant_default11(tau6), padAngle = constant_default11(0);
  function pie(data) {
    var i, n = (data = array_default3(data)).length, j, k2, sum5 = 0, index4 = new Array(n), arcs = new Array(n), a0 = +startAngle.apply(this, arguments), da2 = Math.min(tau6, Math.max(-tau6, endAngle.apply(this, arguments) - a0)), a1, p = Math.min(Math.abs(da2) / n, padAngle.apply(this, arguments)), pa = p * (da2 < 0 ? -1 : 1), v2;
    for (i = 0; i < n; ++i) {
...
// node_modules/d3/node_modules/d3-shape/src/link.js
...
function link2(curve) {
  ...
  function link3() {
    let buffer;
    const argv = slice5.call(arguments);
...
function ascending_default2(series) {
  var sums = series.map(sum4);
  return none_default2(series).sort(function(a4, b) {
    return sums[a4] - sums[b];
  });
}
function sum4(series) {
...
function insideOut_default(series) {
  var n = series.length, i, j, sums = series.map(sum4), order = appearance_default(series), top = 0, bottom = 0, tops = [], bottoms = [];
  for (i = 0; i < n; ++i) {
    j = order[i];
    if (top < bottom) {
      top += sums[j];
...
function zoom_default2() {
  ...
  zoom.scaleBy = function(selection4, k2, p, event) {
    zoom.scaleTo(selection4, function() {
      var k0 = this.__zoom.k, k1 = typeof k2 === "function" ? k2.apply(this, arguments) : k2;
      return k0 * k1;
    }, p, event);
  };
  zoom.scaleTo = function(selection4, k2, p, event) {
    zoom.transform(selection4, function() {
      var e = extent3.apply(this, arguments), t02 = this.__zoom, p02 = p == null ? centroid(e) : typeof p === "function" ? p.apply(this, arguments) : p, p1 = t02.invert(p02), k1 = typeof k2 === "function" ? k2.apply(this, arguments) : k2;
      return constrain(translate(scale2(t02, k1), p02, p1), e, translateExtent);
    }, p, event);
  };
  zoom.translateBy = function(selection4, x4, y4, event) {
    zoom.transform(selection4, function() {
  ...
  zoom.translateTo = function(selection4, x4, y4, p, event) {
    zoom.transform(selection4, function() {
      var e = extent3.apply(this, arguments), t = this.__zoom, p02 = p == null ? centroid(e) : typeof p === "function" ? p.apply(this, arguments) : p;
  ...
  function schedule(transition2, transform2, point6, event) {
    transition2.on("start.zoom", function() {
      ...
      var that = this, args = arguments, g = gesture(that, args).event(event), e = extent3.apply(that, args), p = point6 == null ? centroid(e) : typeof point6 === "function" ? point6.apply(that, args) : point6, w = Math.max(e[1][0] - e[0][0], e[1][1] - e[0][1]), a4 = that.__zoom, b = typeof transform2 === "function" ? transform2.apply(that, args) : transform2, i = interpolate(a4.invert(p).concat(w / a4.k), b.invert(p).concat(w / b.k));
      return function(t) {
        if (t === 1)
          t = b;
        else {
  ...
  function mousedowned(event, ...args) {
    if (touchending || !filter6.apply(this, arguments))
      return;
  ...
  function dblclicked(event, ...args) {
    if (!filter6.apply(this, arguments))
      return;
    var t02 = this.__zoom, p02 = pointer_default(event.changedTouches ? event.changedTouches[0] : event, this), p1 = t02.invert(p02), k1 = t02.k * (event.shiftKey ? 0.5 : 2), t12 = constrain(translate(scale2(t02, k1), p02, p1), extent3.apply(this, args), translateExtent);
    noevent_default3(event);
    if (duration > 0)
      select_default(this).transition().duration(duration).call(schedule, t12, p02, event);
    else
      select_default(this).call(zoom.transform, t12, p02, event);
  }
  ...
  zoom.filter = function(_) {
    return arguments.length ? (filter6 = typeof _ === "function" ? _ : constant_default12(!!_), zoom) : filter6;
  };
  zoom.touchable = function(_) {
    return arguments.length ? (touchable = typeof _ === "function" ? _ : constant_default12(!!_), zoom) : touchable;
  };
  zoom.extent = function(_) {
    return arguments.length ? (extent3 = typeof _ === "function" ? _ : constant_default12([[+_[0][0], +_[0][1]], [+_[1][0], +_[1][1]]]), zoom) : extent3;
  };
...
export {
  ...
  count,
  create_default3 as create,
  creator_default3 as creator,
  cross,
  csv,
  csvFormat,
  csvFormatBody,
  ...
  minIndex,
  mode,
  namespace_default4 as namespace,
  namespaces_default3 as namespaces,
  nice,
  now,
  ...
  pointer_default4 as pointer,
  pointers_default3 as pointers,
  ...
  select_default8 as select,
  selectAll_default7 as selectAll,
  selection_default5 as selection,
  selector_default4 as selector,
  selectorAll_default4 as selectorAll,
